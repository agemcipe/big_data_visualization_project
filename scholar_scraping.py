...
def append_dict_to_file(dict, output_file):
    pd.DataFrame(dict).to_csv(intermediate_dir / "scholar_scraping" / output_file, mode="a", header=False)
...
def process_chunk(author_name_ls):
    ...
    try:
        scholar_data = [fail_safe_search_author(name, verbose=True) for name in author_name_ls]
        append_dict_to_file(scholar_data, f"output_{random.randint(100_000, 999_999)}.csv")
...
if __name__ == "__main__":
    ...
    author_chunks = [author_ls[x:x+chunk_size] for x in range(0, len(author_ls), chunk_size)]
